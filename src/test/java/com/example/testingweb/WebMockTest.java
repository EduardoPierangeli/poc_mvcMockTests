...
import com.example.testingweb.entrypoint.controllers.GreetingController;
...
/**
 * Testes sem subir um servidor, iniciando apenas uma controller(essa anotação desta classe faz isso)
 */
...
public class WebMockTest {
    ...
    @Test
     void greetingShouldReturnMessageFromService() throws Exception {
        when(useCase.greet()).thenReturn("Hello, Mock");
    ...
    @Test
     void testeError() throws Exception {
        when(useCase.teste(Mockito.any(ObjectTestRequest.class)))
                .thenThrow(new CustomException("Error"));
        mockMvc.perform(MockMvcRequestBuilders
                        .post("/teste/{id}", 2)
                        .content(asJsonString(new ObjectTestRequest(5, "dsadasd", 4L)))
                        .contentType(MediaType.APPLICATION_JSON)
